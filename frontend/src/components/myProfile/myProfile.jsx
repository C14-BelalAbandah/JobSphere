...
function myProfile() {
  ...
  useEffect(()=>{
axios.get(`http://localhost:5000/jobs/${userId}`, {headers:{
  ...
  return <div className="myProfilePage">
    {showPostedJobs && <div className="postedJobsSection">
        {myJobs.map((ele,i)=>{
          return (<div key={i}>
            <div>{ele.title}</div>
            <div>{ele.discreption}</div>
            <div>{ele.requirements}</div>
            <div>{ele.location}</div>
            <div>{ele.experince}</div>
            <div>{ele.role}</div>
            <div>{ele.salaryRange}</div>
            <div> {ele.applications}</div>
          </div>)  
        })}
        </div>}
  </div>;
